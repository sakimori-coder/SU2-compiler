...
int main(){
    ...
    // ターゲットユニタリを生成
    set_random_unitary_seed(1234);
    vector<quaternion> U(n);
    for(int i = 0; i < n; i++){
        U[i] = random_unitary();
        cout << setprecision(20) << U[i] << endl;
    }
    ...
    for(auto eps : eps_vec){
        ...
        for(int i = 0; i < n; i++){
            ...
            auto mixed_unitary = Prob_Unitary_Synthesis(U[i], eps);
            end = chrono::system_clock::now();
            time += static_cast<double>(chrono::duration_cast<chrono::microseconds>(end - start).count() / 1000.0);
            int T_count = 0;
            for(auto [p, V] : mixed_unitary){
                T_count = max(T_count, (int)count(V.begin(), V.end(), 'T'));
            }
            AVE += (double)T_count;
            MIN = min(MIN, T_count);
            MAX = max(MAX, T_count);
