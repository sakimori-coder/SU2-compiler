...
void test_cal_inter_2ball(){
    cout << "start test_call_inter2ball" << endl;
    random_device rd;
    default_random_engine eng(rd()); 
    uniform_real_distribution<double> distr_pi(0.0, M_PI);
    uniform_real_distribution<double> distr_2pi(0.0, 2*M_PI);
    ...
    quaternion<double> inter = cal_inter_2ball(u1, u2, eps+1e-6);
    // 交わるはずなので、全て同じ距離が出るはず
    cout << "d(u1, inter) = " << distance(u1, inter) << endl;
    cout << "d(u2, inter) = " << distance(u2, inter) << endl; 
}
