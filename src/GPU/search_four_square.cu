                            ...
                            const T& key)
{
    ...
    thrust::host_vector<T> XY(N1*N2), ZW(N3*N4);
    thrust::device_vector<T> XY_device(N1*N2), ZW_device(N3*N4);
    ...
    // メモリ転送 CPU -> GPU
    thrust::copy(XY.begin(), XY.end(), XY_device.begin());
    thrust::copy(ZW.begin(), ZW.end(), ZW_device.begin());
    // GPU上でソート
    // thrust::sort(XY_device.begin(), XY_device.end());
    // thrust::sort(ZW_device.begin(), ZW_device.end());
    // メモリ転送 GPU -> CPU
    thrust::copy(XY_device.begin(), XY_device.end(), XY.begin());
    thrust::copy(ZW_device.begin(), ZW_device.end(), ZW.begin());
    ...
    // ret.push_back({first_X, first_Y, first_Z, first_W});
